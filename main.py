...
def initialize_entry_exit_times():
    times = {} # Dictionary to store entry and exit times, initialized to None
    for region in counting_regions:
        times[f"{region['name']} entry_time"] = None # Initialize entry time to None
        times[f"{region['name']} exit_time"] = None # Initialize exit time to None
    return times
...
def is_within_region(bbox, polygon):
    """Check if the center of the bounding box is within the polygon region."""
    x_center = (bbox[0][0] + bbox[0][2]) / 2  # Calculate the center of the bounding box
    y_center = (bbox[0][1] + bbox[0][3]) / 2  # Calculate the center of the bounding box
...
def run(weights="model/PeopleDetector.pt", 
        ...
        track_thickness=2, 
        region_thickness=2):
    """Run Region counting on a video using YOLOv8 custom model and ByteTrack."""
    vid_frame_count = 0
    ...
    while videocapture.isOpened():
        ...
        for region in counting_regions:
            ...
            # Display counters in the upper right corner
